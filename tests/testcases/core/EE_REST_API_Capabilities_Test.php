...
class EE_REST_API_Capabilities_Test extends EE_UnitTestCase{
		...
		return array(
			'Event' => array(
				WP_JSON_Server::READABLE => array(
					...
					'EVT_wp_user' => array(
						'can_see_wp_user' => new EE_API_Access_Entity_If_Owner()//...except you need to be the owner to see the wp user, or have the 'can_see_wp_user' cap
					)
				)
			),
			'Registration' => array(
				WP_JSON_Server::READABLE => array(
					'*' => array(
						'mambo' => new EE_API_Access_Entity_If_Owner()
					),
					'REG_att_is_going' => array(
						'ee_read_checkins' => new EE_API_Access_Entity_Never()
					)
				)
			),
			'Attendee' => array(
				WP_JSON_Server::READABLE => array(
					'*' => array(
						'foobar' => new EE_API_Access_Entity_If( array( 'ATT_fname' => 'foobar' ) )
					)
				)
			),
			'Question' => array(
			...
			'Venue' => array(
				WP_JSON_Server::READABLE => array(
					'*' => array(
						'access_venues' => new EE_API_Access_Entity_Never()
					)
				)
			)
		);
	}
	function test_add_restrictions_onto_query__basic_if_owner(){
		$new_params = EE_REST_API_Capabilities::add_restrictions_onto_query( array(), EE_Registry::instance()->load_model( 'Registration') );
		$this->assertEquals(
				array(
					array(
						'Event.EVT_wp_user' => 0
	...
	function test_add_restrictions_onto_query__basic_other_restrictions(){
		$new_params = EE_REST_API_Capabilities::add_restrictions_onto_query( array(), EE_Registry::instance()->load_model( 'Attendee') );
		$this->assertEquals(
				array(
					array(
						'ATT_fname' => 'foobar'
	...
	function test_add_restrictions_onto_query__preexisting_query_overwrite() {
		$old_params = array( array( 'Event.EVT_wp_user' => 123) );
		$new_params = EE_REST_API_Capabilities::add_restrictions_onto_query( $old_params, EE_Registry::instance()->load_model( 'Registration') );
		//old query gets ovewritten
		$this->assertEquals(
				array(
					array(
						'Event.EVT_wp_user' => 0
	...
	function test_add_restrictions_onto_query__misconfigured_restrictions_array() {
		try{
			EE_REST_API_Capabilities::add_restrictions_onto_query( array(), EE_Registry::instance()->load_model( 'Question') );
			$this->asserTrue( false );
		}catch( EE_Error $e ){
			//we expected an error becaues we have setup the restrctions array wrong for this model
